...
@router.get("/stream")
async def stream(room: str = Query(default=_default_room), token: str = Query(default="")):
    """SSE endpoint. Token passed as query param since EventSource doesn't support headers."""
    # Verify token manually (EventSource can't set Authorization header)
    if not token:
