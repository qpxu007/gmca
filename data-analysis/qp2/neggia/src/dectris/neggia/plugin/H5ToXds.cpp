...
struct H5DataCache {
    ...
    int dimy = 0;
    int datasize = 0;
    int nframesPerDataset = 0;
    std::unique_ptr<int32_t[]> mask;
    float xpixelSize = 0;
    float ypixelSize = 0;
    bool masterFileOnly = false;
};
...
void setPixelMask(H5DataCache* dataCache) {
    try {
        ...
        try {
            ...
            dataCache->dimx = (int)readNonZeroUint(x_pixels);
            dataCache->dimy = (int)readNonZeroUint(y_pixels);
            size_t s = (size_t)(dataCache->dimx * dataCache->dimy);
            dataCache->mask.reset(new int32_t[s]());
        } catch (const std::out_of_range&) {
            // we could try to determine from data frames, but that seems brittle
        }
    }
}
...
void setNFramesPerDatasetFromPath(H5DataCache* dataCache,
    ...
    try {
        ...
        if (dataCache->dimx == 0 && dataCache->dimy == 0) {
            dataCache->dimy = dim[1];
            dataCache->dimx = dim[2];
            size_t s = (size_t)(dataCache->dimx * dataCache->dimy);
            dataCache->mask.reset(new int32_t[s]());
        } else {
